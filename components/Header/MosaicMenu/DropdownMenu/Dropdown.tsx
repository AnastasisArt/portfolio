...
export default function Dropdown({ rendered, open, topPx, leftPx, children, contacts }: Props) {
  if (typeof document === 'undefined' || !rendered) return null;

  //Render outside the DOM tree to avoid overflow issues
  return createPortal(
    <DropdownWrap className={open ? "open" : "close"} style={{ top: `${topPx}px`, left: `${leftPx}px` }}>
      <DropdownContent>
        {children}
